...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-2xl ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
      ...
      <main className="min-h-screen bg-[var(--editable-page-bg,#fff7ee)] text-[var(--editable-page-text,#2f1d16)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="fusion-section-rule">
            <span className="fusion-section-label">Search</span>
          </div>
          <div className="mt-8 grid gap-8 border-b-4 border-black bg-white md:grid-cols-[0.8fr_1.2fr]">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] opacity-55">{pagesContent.search.hero.badge}</p>
              <h1 className="editable-display mt-5 text-5xl font-semibold leading-[1.02] sm:text-7xl">{pagesContent.search.hero.title}</h1>
              <p className="mt-6 max-w-xl text-base font-semibold leading-8 opacity-70">{pagesContent.search.hero.description}</p>
            </div>
            <form action="/search" className="self-end border-4 border-black bg-[var(--slot4-warm)] p-4 shadow-[10px_10px_0_rgba(213,54,43,0.18)] sm:p-5">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center bg-[var(--editable-page-text,#2f1d16)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--editable-page-bg,#fff7ee)] transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="editable-display mt-2 text-4xl font-semibold">{query ? `Results for "${query}"` : pagesContent.search.resultsTitle}</h2>
            </div>
          ...
          {results.length && results.length <= 6 ? (
